...
### 看运行中任务的输出（dispatch 后回头瞄）

dispatch 返回的 worker 日志首行里有 `task <task_id> started`。拿该 task_id 可随时查：

```bash
# 拉一次快照（状态 + 到目前为止的正文 + 在飞工具 + 计数器）
node ~/.agents/skills/pi-mcp-claude/scripts/mcp_claude.mjs status <task_id>

# 流式跟看到终态（逐字吐正文，[tool] 打到 stderr）
node ~/.agents/skills/pi-mcp-claude/scripts/mcp_claude.mjs watch <task_id> [--from-seq N]
```

- `status` 底层 `claude_status`：返回快照 `text`（累积正文）/`currentTool`/`counters`，轮询用。
- `watch` 底层 `claude_wait` long-poll：从游标后逐段流出事件（text 增量 / tool_use / error），任务达终态自动返回。`--json` 可输出原始事件记录。
- 两者都只读、不影响任务运行；多个客户端（Pi/Hermes）可同时 watch 同一任务。
