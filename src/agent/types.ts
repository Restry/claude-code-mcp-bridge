...
export interface AgentRunOptions {
   ...
   * the returned run. Lets the agent (and any subprocess it spawned, e.g. a
   * long-running Bash) clean up before the kernel reaps the tree.
   * Adapters that don't kill via signals are free to ignore this. Defaults
   * are adapter-specific.
   */
  stopGraceMs?: number;
  /**
   * Override `--append-system-prompt`. When undefined or null the flag is
   * skipped entirely; pass a string to inject a system prompt.
   */
